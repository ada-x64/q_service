...
/// [A Space-Efficient Algorithm for Finding Strongly Connected Components][1]
/// by David J. Pierce, which is a memory-efficient variation of [Tarjan's
/// algorithm][2].
///
///
...
pub(crate) fn new_tarjan_scc(
    ...
    TarjanScc {
        ...
        component_count: usize::MAX, /* Will hold if component_count is
                   * initialized to number of nodes - 1 or
                   * higher. */
        nodes,
        stack: Vec::new(),
...
/// A state for computing the *strongly connected components* using [Tarjan's
/// algorithm][1].
///
...
{
    /// Source of truth [`DependencyGraph`]
    graph: &'graph DependencyGraph,
    /// An [`Iterator`] of [`NodeId`]s from the `graph` which may not have been
    /// visited yet.
    ...
    /// Information about each [`NodeId`], including a possible SCC index and
    /// an [`Iterator`] of possibly unvisited neighbors.
    nodes: Vec<NodeData<Neighbors>>,
    /// A stack of [`NodeId`]s where a SCC will be found starting at the top of
    /// the stack.
    stack: Vec<NodeId>,
    /// A stack of [`NodeId`]s which need to be visited to determine which SCC
    /// they belong to.
    visitation_stack: Vec<(NodeId, bool)>,
    /// An index into the `stack` indicating the starting point of a SCC.
    start: Option<usize>,
    /// An adjustment to the `index` which will be applied once the current SCC
    /// is found.
...
impl<'graph, A: Iterator<Item = NodeId>, N: Iterator<Item = NodeId>> TarjanScc<'graph, A, N> {
    ...
    /// variation of [Tarjan's algorithm][2].
    ///
    ///
    /// [1]: https://homepages.ecs.vuw.ac.nz/~djp/files/P05.pdf
    /// [2]: https://en.wikipedia.org/wiki/Tarjan%27s_strongly_connected_components_algorithm
    ...
    fn next_scc(&mut self) -> Option<&[NodeId]> {
        ...
        loop {
            ...
            // finished visiting the node at the bottom of the stack
            // yet. Must visit all nodes in the stack from top to
            // bottom before visiting the next node.
            ...
            // Get the next node to check, otherwise we're done and can return
            // None.
    ...
    /// visiting neighbors. If a visitation is required, this will return
    ...
    fn visit_once(&mut self, v: NodeId, mut v_is_local_root: bool) -> Option<usize> {
        ...
        while let Some(w) = self.nodes[self.graph.to_index(v)].neighbors.next() {
            ...
            if self.nodes[self.graph.to_index(w)].root_index.is_none() {
                ...
                // visitation stack. On the next execution of
                // `visit_once`, the neighbor will be visited.
...
impl<'graph, A: Iterator<Item = NodeId>, N: Iterator<Item = NodeId>> Iterator
    for TarjanScc<'graph, A, N>
{
    // It is expected that the `DiGraph` is sparse, and as such wont have many
    // large SCCs. Returning a `SmallVec` allows this iterator to skip
